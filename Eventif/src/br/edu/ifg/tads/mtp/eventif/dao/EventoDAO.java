...
public class EventoDAO {
	 ...
	 * RuntimeException(e); } }
	 * 
	 * public void altera(EventoModel evento) { String sql =
	 * "update eventos set nome=?, email=?, endereco=?," +
	 * "datanascimento=? where id=?"; try { PreparedStatement stmt = new
	 * ConnectionFactory().getConnection().prepareStatement(sql);
	 * stmt.setString(1, evento.getNome()); stmt.setDate(4, new
	 * Date(evento.getData() .getTimeInMillis())); stmt.setLong(5,
	 * evento.getId()); System.out.println(stmt.toString());
	 * stmt.executeUpdate(); stmt.close(); } catch (SQLException e) {
	 * System.out.println(e); throw new RuntimeException(e); } }
	 * 
	 * public void remove(EventoModel evento) { try { PreparedStatement stmt =
	 * new ConnectionFactory().getConnection().prepareStatement(
	 * "delete from eventos where id=?"); stmt.setLong(1, evento.getId());
